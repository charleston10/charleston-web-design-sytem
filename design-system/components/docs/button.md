# Button (Web Construction Guide)

Este documento descreve as especificações técnicas e diretrizes de arquitetura para a construção do componente `Button` para a plataforma Web, seguindo os padrões de excelência do Charleston Design System.

## 🎯 Objetivo

Componentes de botão variados. Na Web, deve ser otimizado para performance, acessibilidade (WAI-ARIA) e consistência visual, utilizando o padrão de **Compound Components**.

## 🏗️ Anatomia e Composição

A estrutura deve permitir controle total sobre a renderização:

1.  **Button.Root**: Container principal (geralmente um `<button>`).
2.  **Button.Label**: Texto interno do botão.
3.  **Button.Icon**: Slot para ícones (opcional).
...
-   **Roles:** Aplicar `role` apropriado conforme a especificação W3C/WAI-ARIA para Button.
