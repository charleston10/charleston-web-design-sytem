# FloatingButton (Web Construction Guide)

Este documento descreve as especificações técnicas e diretrizes de arquitetura para a construção do componente `FloatingButton` para a plataforma Web, seguindo os padrões de excelência do Charleston Design System.

## 🎯 Objetivo

Botão flutuante para ações de destaque. Na Web, deve ser otimizado para performance, acessibilidade (WAI-ARIA) e consistência visual, utilizando o padrão de **Compound Components**.

## 🏗️ Anatomia e Composição

A estrutura deve permitir controle total sobre a renderização:

1.  **FloatingButton.Root**: Container principal que gerencia o estado e contexto.
2.  **FloatingButton.Content**: Onde o conteúdo principal é renderizado.
...
### Propriedades (FloatingButton.Root)
...
-   **Roles:** Aplicar `role` apropriado conforme a especificação W3C/WAI-ARIA para FloatingButton.

## 📂 Estrutura de Arquivos Recomendada

```
floating-button/
├── floating-button.root.tsx
├── floating-button.content.tsx
├── floating-button.context.ts
├── floating-button.types.ts
├── floating-button.styles.ts
└── README.md
```
